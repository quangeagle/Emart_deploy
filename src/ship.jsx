...
const Shipping = () => {
  ...
  const handleSubmit = () => {
    axios.post('http://localhost:3004/ship/themship', { 
      ...
      orderItems: cartItems
        ...
        .map(item => ({
          productId: item.productId._id, // Safe to access _id now
          quantity: item.quantity
        }))
    })
      .then(response => {
  ...
  {cartItems.length > 0 ? (
    cartItems.map(item => (
      item.productId?.name && item.productId?.price && item.productId?.imageUrl ? (  // Kiểm tra tên, giá và hình ảnh của sản phẩm
