...
function AccountInfo() {
  ...
  useEffect(() => {
    if (user.id) {
      ...
      setError(null); // Xóa lỗi trước đó
      axios
        .get(`http://localhost:3005/ship/${user.id}`)
        .then((response) => {
  ...
  const handleCancelOrder = (orderId) => {
    // Xử lý hủy đơn hàng
    axios
      .put(`http://localhost:3007/ship/cancel`, {
        orderId,
      })
      .then((response) => {
  ...
  const handleRefundOrder = async (orderID) => {
    try {
      ...
      const response = await fetch("http://localhost:3007/ship/refund", {
        ...
        headers: {
          Authorization:
            "Bearer sk_pointerf97ad5e90eb156b9a2b5d18e44bb37f8c89c2f0db611038a751c3bc7e0ec63c6",
  ...
  return (
    <div className="p-4">
      <h2 className="mb-4 text-2xl font-bold">Thông Tin Tài Khoản</h2>
      <div className="mb-6">
        <h3 className="text-xl font-semibold">Lịch Sử Mua Hàng</h3>
        {loading ? (
          <p>Đang tải dữ liệu...</p> // Hiển thị trạng thái đang tải
          ...
          <table className="mt-4 w-full border-collapse border">
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order._id}>
                  <td className="border p-2 text-center">{order._id}</td>
                  ...
                  <td className="border p-2 text-center">
                    {order.orderItems.map((item) => (
                      <div key={item._id}>{item.price} VND</div>
                    ))}
                  </td>
                  <td className="border p-2 text-center">
                  ...
                  <td className="border p-2 text-center">
                    {order.status === "pending" && (
                      <button
                        ...
                        className="rounded bg-red-500 px-4 py-2 text-center text-white"
                      >
                        Hủy
                      </button>
                    )}
